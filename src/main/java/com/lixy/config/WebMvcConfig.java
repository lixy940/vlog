...
import com.lixy.web.interceptor.IndexInterceptor;
import com.lixy.web.interceptor.InstallInterceptor;
import com.lixy.web.interceptor.LoginInterceptor;
...
//				.excludePathPatterns("/admin/getLogin");
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		if (System.getProperty(StaticParameterUtils.SYS_PROPERTY_OS_NAME).toLowerCase().contains(StaticParameterUtils.SYS_OS_NAME_WINDOW)) {
			...
			.addResourceLocations("file:///"+ StaticParameterUtils.UPLOAD_PIC_WINDOWS);
			File file = new File(StaticParameterUtils.UPLOAD_PIC_WINDOWS);
			if (!file.exists()) {
				file.mkdirs();
			}
		}else{
			registry.addResourceHandler("/upload/**").addResourceLocations("classpath:/upload/")
					.addResourceLocations("file:///"+StaticParameterUtils.UPLOAD_PIC_LINUX);
			File file = new File(StaticParameterUtils.UPLOAD_PIC_LINUX);
			if (!file.exists()) {
				file.mkdirs();
			}
		}
