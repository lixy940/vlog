package com.lixy.web.controller.front;
...
import com.lixy.model.enums.PageNumber;
import com.lixy.model.enums.PostType;
import com.lixy.util.MaydayUtil;
import com.lixy.web.controller.admin.BaseController;
...
import com.lixy.service.CategoryService;
...
public class IndexController extends BaseController {
	...
	public boolean checkRepeatIp(HttpServletRequest request, int id) {
		...
		Integer count = (Integer)redisService.hget("hits:frequency", value);
		if (count != null && count > 0) {
			return true;
		}
		redisService.hset("hits:frequency", value, 1, MaydayConst.IP_REPEAT_TIME, TimeUnit.MILLISECONDS);
		return false;
	}
	...
	public void updateArticleViews(Integer id, Long views) {
		...
		// 获取缓存数据
		Long cacheViews = (Long) redisService.hget("article" + id, "cacheViews");
		// 如果缓存数据为null赋值1，反之加1
		...
		if (cacheViews >= MaydayConst.CLICK_EXCEED) {
			...
			redisService.hset("article" + id, "cacheViews", null);
		} else {
			redisService.hset("article" + id, "cacheViews", cacheViews);
		}
	}
